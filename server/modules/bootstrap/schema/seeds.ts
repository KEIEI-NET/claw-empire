...
import { seedDefaultPersonas } from "./persona-seeds.ts";
...
export function applyDefaultSeeds(db: DbLike): void {
  ...
  if (agentCount === 0) {
    ...
    // Development (3)
    insertAgent.run(randomUUID(), "Aria", "아리아", "dev", "team_leader", "claude", "👩‍💻", "几帳面なシニア開発者");
    insertAgent.run(randomUUID(), "Bolt", "볼트", "dev", "senior", "codex", "⚡", "高速コーディングの専門家");
    insertAgent.run(randomUUID(), "Nova", "노바", "dev", "junior", "copilot", "🌟", "創造的なジュニア");
    // Design (2)
    insertAgent.run(randomUUID(), "Pixel", "픽셀", "design", "team_leader", "claude", "🎨", "デザインリーダー");
    insertAgent.run(randomUUID(), "Luna", "루나", "design", "junior", "gemini", "🌙", "感性豊かなUIデザイナー");
    // Planning (2)
    insertAgent.run(randomUUID(), "Sage", "세이지", "planning", "team_leader", "codex", "🧠", "戦略アナリスト");
    insertAgent.run(randomUUID(), "Clio", "클리오", "planning", "senior", "claude", "📝", "データドリブンなプランナー");
    // Operations (2)
    insertAgent.run(randomUUID(), "Atlas", "아틀라스", "operations", "team_leader", "claude", "🗺️", "運用の達人");
    insertAgent.run(randomUUID(), "Turbo", "터보", "operations", "senior", "codex", "🚀", "自動化の専門家");
    // QA/QC (2)
    insertAgent.run(randomUUID(), "Hawk", "호크", "qa", "team_leader", "claude", "🦅", "鋭い品質監視者");
    insertAgent.run(randomUUID(), "Lint", "린트", "qa", "senior", "codex", "🔬", "几帳面なテスト専門家");
    // DevSecOps (2)
    insertAgent.run(randomUUID(), "Vault", "볼트S", "devsecops", "team_leader", "claude", "🛡️", "セキュリティアーキテクト");
    insertAgent.run(randomUUID(), "Pipe", "파이프", "devsecops", "senior", "codex", "🔧", "CI/CDパイプラインの専門家");
    // QA Junior (1)
    insertAgent.run(randomUUID(), "DORO", "도로롱", "qa", "junior", "gemini", "🩷", "几帳面な品質管理ジュニア");
  ...
  // Migrate: add sort_order column & set correct ordering for existing DBs
  {
    ...
    const newAgents: [string, string, string, string, string, string, string][] = [
      ...
      ["Luna", "루나", "design", "junior", "gemini", "🌙", "感性豊かなUIデザイナー"],
      ["Clio", "클리오", "planning", "senior", "claude", "📝", "データドリブンなプランナー"],
      ["Turbo", "터보", "operations", "senior", "codex", "🚀", "自動化の専門家"],
      ["Hawk", "호크", "qa", "team_leader", "claude", "🦅", "鋭い品質監視者"],
      ["Lint", "린트", "qa", "senior", "opencode", "🔬", "几帳面なテスト専門家"],
      ["Vault", "볼트S", "devsecops", "team_leader", "claude", "🛡️", "セキュリティアーキテクト"],
      ["Pipe", "파이프", "devsecops", "senior", "codex", "🔧", "CI/CDパイプラインの専門家"],
    ];
